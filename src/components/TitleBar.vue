  ...
  <header class="titlebar">
    ...
    <div class="tb-brand">
      ...
      <div>
        ...
        <div class="logo-sub">魏家峁煤矿 · 运煤皮带智能监控</div>
      </div>
    </div>
...
const loadClass = computed(() => {
  if (store.metrics.load > 2000) return 'warn'
  return ''
})
...
.tb-tab:hover { color: var(--text-pri); background: var(--bg-hover); }
.tb-tab.active {
  color: var(--accent);
...
.metric-value.danger { color: var(--danger); animation: blink 0.6s step-end infinite; }
...
.alarm-bell:hover   { border-color: var(--warn); color: var(--warn); }
.alarm-bell.ringing { border-color: rgba(255,59,92,.4); color: var(--danger); animation: blink 0.8s step-end infinite; }
.bell-badge {
...
.sensor-leds { display: flex; gap: 6px; }
.led-item {
  font-family: var(--font-mono);
